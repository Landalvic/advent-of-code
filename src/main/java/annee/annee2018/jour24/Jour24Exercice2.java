package annee.annee2018.jour24;
	...
	public static void main(String[] args) {
		...
		for (Groupe groupe : groupes) {
			if (groupe.isSystemeImmunitaire()) {
				groupe.setAttaque(groupe.getAttaque() + nbrBoost);
			}
		}
		while (!isSystemeImmunitaireVainqueur(groupes)) {
			nbrBoost++;
			for (Groupe groupe : groupes) {
				groupe.initialiserCombat();
				if (groupe.isSystemeImmunitaire()) {
					groupe.setAttaque(groupe.getAttaque() + 1);
				}
			}
		...
		System.out.println(nbrBoost);

	}

	private static boolean isSystemeImmunitaireVainqueur(List<Groupe> groupes) {
