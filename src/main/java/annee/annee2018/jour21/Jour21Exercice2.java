package annee.annee2018.jour21;
...
public class Jour21Exercice2 {
	...
	public static void main(String[] args) {
		...
		List<Long> dejaVu = new ArrayList<>();
		while (true) {
			if (instructionPointer < 0 || instructionPointer >= operations.size()) {
				break;
			}
			if (instructionPointer == 26) {
				dejaVu.add(registres[2]);
				System.out.println(registres[2] + " - " + registres[4]);
			}
	...
	public static void afficherBinaire(int nombre) {
		String test = Integer.toBinaryString(nombre);
		while (test.length() < 50) {
			test = "0" + test;
		}
