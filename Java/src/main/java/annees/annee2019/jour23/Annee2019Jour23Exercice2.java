...
public class Annee2019Jour23Exercice2 extends Exercice {

	public static void main(String[] args) {
		new Annee2019Jour23Exercice2().lancer("src/main/resources/annee2019/jour23/data.txt");
	}
	...
	public String run(String input) throws AdventOfCodeException {
		...
		Set<Long> nats = new HashSet<Long>();
		Operation nat = null;
		for (int i = 0; i < 50; i++) {
		...
		while (!allCodeFinished(codes)) {
			if (operations.isEmpty()) {
					...
					if (!retour.isEmpty()) {
						if (retour.get(0).intValue() == 255) {
							nat = new Operation(retour.get(0).intValue(), retour.get(1), retour.get(2),
									code.getNbrEtape());
							if (!nats.add(nat.getY())) {
								System.out.println("yoooo:" + nat.getY());
							}
				...
				if (operations.isEmpty()) {
					...
					System.out.println("oui!" + retour);
					if (retour.get(0).intValue() == 255) {
						if (!nats.add(nat.getY())) {
							System.out.println("yooo:" + nat.getY());
						}
						System.out.println("y1:" + nat.getY());
			...
			if (!retour.isEmpty()) {
				if (retour.get(0).intValue() == 255) {
					nat = new Operation(retour.get(0).intValue(), retour.get(1), retour.get(2), code.getNbrEtape());
					if (!nats.add(nat.getY())) {
						System.out.println("yooo:" + nat.getY());
					}
				} else {
					operations.add(
							new Operation(retour.get(0).intValue(), retour.get(1), retour.get(2), code.getNbrEtape()));
				}
			}
		}
		return "";
