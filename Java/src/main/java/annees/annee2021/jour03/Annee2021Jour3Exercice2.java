...
public class Annee2021Jour3Exercice2 extends Annee2021Jour3 {
	...
	public String run(String input) throws AdventOfCodeException {
		var liste = FileUtils.listOfCharacters(input);
		var gamma = calcul(liste, 0, "1", "0");
		var epsilon = calcul(liste, 0, "0", "1");
		return String.valueOf(gamma * epsilon);
	}

	private long calcul(List<List<String>> liste, int index, String charPrincipal, String charAEnlever) {
		if (liste.size() == 1) {
			return MathUtils.binaryToLong(String.join("", liste.get(0)), 2);
		}
		var position = new Position(0, 0);
		liste.forEach(list -> updatePosition(list.get(index), position));
		String charAVerifier = position.getX() >= position.getY() ? charPrincipal : charAEnlever;
		List<List<String>> newListe = liste.stream().filter(s -> StringUtils.equals(s.get(index), charAVerifier)).toList();
